...
BEGIN
    ...
    WITH buffer_per_day AS (
        SELECT
            ...
            (ST_Dump(
                -- se os focos estiverem proximos a ponto de obreor formaremos um unico poligono
                ST_Union(
                    --criar um quadrado de 500m x 500m metros ao redor de cada ponto
                    --geometria do ponto convertido para geography
                    --ponto mais buffer convertido em cartesiano
                    ST_Buffer(f.geom::geography, 500, 'endcap=square')::geometry
                )
            )).geom AS geomk
        -- define o f como alias de evento_fogo_teste.focos    
    ...
    dissolved_fire_fronts AS (
        SELECT 
            o.data_frente,
            -- ST_Union(o.geomk) une as geometrias do dia em um balde MULTPOLYGON
            -- ST_Dump separa o MULTIPOLYGON em POLYGONs individuais
